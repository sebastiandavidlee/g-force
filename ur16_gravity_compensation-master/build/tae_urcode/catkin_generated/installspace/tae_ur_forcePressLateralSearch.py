...
def saveDataParams(dutyInfo, FzTarget,xAdjust):
  ...
  savingDictionary["xAdjust"]= xAdjust
  ...
  savingFileName = ResultSavingDirectory + '/' + savingFileName_noDir + '_Fz_' + str(FzTarget) + '_' + positionStr + '_' + str(dutyInfo[0]) + 'Hz' + str(dutyInfo[1]) +'duty'
...
def main(rotateAngle, FzTarget, freq, duty, iterNum, xAdjust=0):
  global prevIphoneFileName
  global thisForce
  global averageFz
  ...
  engagePosition = [-0.3805, .311, 0.280]
  engagePosition[0] += xAdjust
  ...
  try:
    ...
    for trialIdx in range(iterNum):
      ...
      try:
        ...
        savingFolder, savingFileName = saveDataParams(PWM_Freq_Duty,FzTarget,xAdjust)    
      except Exception as e:
        print e
...
if __name__ == '__main__':  
  ...
  if not autoMode:
    ...
    xAdjustList = [0.015, 0, -0.015]
    freq_dutyList = [[100,100],[100, 60], [30,60], [100,30], [30, 30]]
    
    print "AutoMode!!"
    for [freq, duty] in freq_dutyList:
      for xAdjust in xAdjustList:      
        main(angle, force, freq, duty, iterNum, xAdjust)
