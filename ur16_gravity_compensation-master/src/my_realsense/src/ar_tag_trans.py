...
def getPos():
    ...
    while not rospy.is_shutdown():
        try:
            ...
            trans_ca = tfBuffer.lookup_transform("camera_link", "ar_marker_0", rospy.Time())
            ...
            g_tc = np.eye(4)
            g_tc[0:3, 0:3] = np.array([[0, 1, 0],
                                       [0, 0, 1],
                                       [1, 0, 0]])
            g_tc[0][3] = rospy.get_param("~cam/x_offset")
            g_tc[1][3] = rospy.get_param("~cam/y_offset")
            g_tc[2][3] = rospy.get_param("~cam/z_offset")
            ...
            quat_bt = trans_bt.transform.rotation
            rot_bt = R.from_quat([quat_bt.x,quat_bt.y,quat_bt.z,quat_bt.w])
            # print('rot_bt:')
            # print(rot_bt.as_dcm())
