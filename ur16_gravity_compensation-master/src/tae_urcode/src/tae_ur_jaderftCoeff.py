...
def saveDataParams(args, betaDList, gammaDList):
  ...
  elif intruderType == 30:
    savingDictionary["intruder"] = '30deg'
  elif intruderType == 60:
    savingDictionary["intruder"] = '60deg'
  elif intruderType == 90:
    savingDictionary["intruder"] = '90deg'

  savingDictionary["beta"] = intruderType * betaIndex
  ...
  savingFileName = ResultSavingDirectory + '/' + savingFileName_noDir + '_intruder_' + str(intruderType) + '_beta_' + str(intruderType * betaIndex) + '_gamma_' + str(gammaDList[gammaIndex])
...
def main(args):
  ...
  gammaDList = [90.0, 71.565, 56.310, 45, 33.690, 18.435, 0] # Angle will be intruding and extruding in the same Path
  ...
  try:
    ...
    waypoints, rotatedInitRobotPose = UR_Interface.getRotatedRobotPose(copy.deepcopy(engageRobotPose),rotateAngle/2,[0, 0, 0],rotateAxis)
    ...
    # waypoints, rotatedInitRobotPose = UR_Interface.getRotatedRobotPose(copy.deepcopy(engageRobotPose),rotateAngle,translateVector,rotateAxis)
    if not UR_Interface.go_to_PoseGradually(rotatedInitRobotPose,speedScale=0.9, wantWait=True):
      print UR_Interface.go_to_PoseGradually(rotatedInitRobotPose,speedScale=0.3,wantWait=True)
    ...
    waypoints, rotatedInitRobotPose = UR_Interface.getRotatedRobotPose(copy.deepcopy(engageRobotPose),rotateAngle/2,[0, 0, 0],rotateAxis)
    # waypoints, rotatedInitRobotPose = UR_Interface.getRotatedRobotPose(copy.deepcopy(engageRobotPose),rotateAngle,translateVector,rotateAxis)
    if not UR_Interface.go_to_PoseGradually(rotatedInitRobotPose,speedScale=0.7, wantWait=True):
      print UR_Interface.go_to_PoseGradually(rotatedInitRobotPose,speedScale=0.1,wantWait=True)
   

    if not UR_Interface.go_to_PoseGradually(engageRobotPose,speedScale=0.9, wantWait=True):
