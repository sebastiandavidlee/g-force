...
def saveDataParams(dutyInfo, FzTarget):
...
def main(rotateAngle, FzTarget, freq, duty, iterNum):
  ...
  try:
    ...
    for trialIdx in range(iterNum):
      print "Go to first engage Position"
      pressStartPosition = engagePosition[:]
      ...
      try:
        ...
        if FT_SimulatorOn:
          netftSimCall(2,0, -0.03, FzTarget)
          rospy.sleep(2)

        zMove = -0.01 #initSearch 1cm
        
        initFz = averageFz
        desiredPosition = pressStartPosition[:]   

        if FT_SimulatorOn:
          netftSimCall(2,4, -0.03, FzTarget)     

        for zIdx in range(50):
          print "search Iterate: ", zIdx, "Zmove: ", zMove
          desiredPosition[2] += zMove

          if UR_Interface.go_to_Position(desiredPosition,speedScale=0.3, wantWait=True): #go to target pose success        
            rospy.sleep(1)
        ...
        if not UR_Interface.go_to_Position(desiredPosition,speedScale=0.3, wantWait=True):
          print UR_Interface.go_to_Position(desiredPosition,speedScale=0.3, wantWait=True)
        
        # Move to the other goal lateral Position while updating the Z target.
        goalPosition = desiredPosition[:]
        goalPosition[0] += scanLateralLen

        InitTic = rospy.get_rostime()
        
        zMove = 0.0

        if FT_SimulatorOn:
          netftSimCall(2,5, 0.3, FzTarget) # Sinusoidal     
          rospy.sleep(2)

        while rospy.get_rostime().secs - InitTic.secs < runtime:
          UR_Interface.go_to_Position(goalPosition, speedScale=0.05, wantWait=False)
          rospy.sleep(0.2)
          thisFz = averageFz-initFz
          zMove += pGain * (FzTarget - thisFz)  
          print zMove
          goalPosition[2] += zMove
        ...
        goalPosition[2] += 0.03      
        UR_Interface.go_to_Position(goalPosition,wantWait=True)
        ...
        savingFolder, savingFileName = saveDataParams(PWM_Freq_Duty,FzTarget)    
      except Exception as e:
        print e
...
if __name__ == '__main__':  
  ...
  parser.add_argument('--freq', type=int, help='force target', default=30)
  parser.add_argument('--duty', type=int, help='force target', default=30)
  parser.add_argument('--iter', type=int, help='force target', default=1)
